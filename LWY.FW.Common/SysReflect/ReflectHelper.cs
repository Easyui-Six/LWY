...
namespace LWY.FW.Common
{
    ...
    public class ReflectHelper
    {
        ...
        #region  获取成员的字段字符串   成员的名字

        public List<string> GetClassMembers<T>() where T : class
        {
            List<string> columns = new List<string>();

            Type t = GetPropertyType(typeof(T));
            MemberInfo[] mbs = t.GetMembers();
        ...
        #region 用dll文件的名字  和要执行的方法做入参 取值
        ...
        public T GetResultByDllName<T>(string dllName, string className, string actionName, object[] param)
        {
            ...
            Assembly assembly = Assembly.LoadFile(AppDomain.CurrentDomain.BaseDirectory + "bin//" + $"{dllName}.dll");
